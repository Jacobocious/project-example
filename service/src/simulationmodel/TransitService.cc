 ...
 public:
  ...
  void receiveCommand(const std::string& cmd, const JsonObject& data,
    ...
    if (cmd == "CreateEntity") {
      ...
      DataDashboard::getInstance()->exportToCSV(filename);
      std::cout << "Exporting data to file: " << filename << "\n";
    } else if (cmd == "stopSimulation") {
      std::cout << "Stop command administered\n";
      stopped = true;
      std::cout << "Exporting final data to file: sim_data_final.csv\n";
      DataDashboard::getInstance()->exportToCSV("sim_data_final.csv");
      model.stop();
    }
  }
  ...
  // generates unique file names per sim session
