...
class IEntity : public IPublisher {
 public:
  ...
  /**
   * @brief Returns the position of the entity as a string.
   * @param position The position of the entity as a Vector3 object.
  */
  virtual std::string formatPosition(const Vector3& position);

  /**
   * @brief Initializes the EntityData structure with the entity's current trip information.
  */
  virtual void initializeTrip();

  /**
   * @brief Updates the entity's current trip data.
   * @param dt The change in time to calculate other changes.
  */
  virtual void updateTrip(double dt);

  /**
   * @brief Stores the destination in entityStorage.
  */
  virtual void storeDestination();
