...
class Package : public IEntity {
 public:
  ...
  /**
   * @brief Stores the package type and pairs ID with drone.
   * @param id The ID to be paired.
   */
  virtual void storeTypeAndId(int id);

  /**
   * @brief Sends package data to DataDashboard.
   */
  virtual void sendPackageData();

  /**
   * @brief Stores the strategy name once picked up.
   */
  virtual void storeMovingStrat();
